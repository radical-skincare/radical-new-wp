      ...
      <div id="review_form_wrapper" class="row justify-content-center">
        <div id="review_form" class="col-lg-10">
          ...
          $comment_form = array(
            ...
            'comment_notes_after' => '',
            'label_submit'        => esc_html__( 'Submit Rating', 'woocommerce' ),
            'logged_in_as'        => '',
            'comment_field'       => '',
          );
          ...
          $fields = array(
            'author' => array(
              'label'    => __( 'Your Full Name', 'woocommerce' ),
              'type'     => 'text',
          ...
          foreach ( $fields as $key => $field ) {
            $field_html = '<div class="comment-form-' . esc_attr( $key ) . ' form-outline mb-3">';
              $field_html .= '<label for="' . esc_attr( $key ) . '">' . esc_html( $field['label'] );
                if ( $field['required'] ) {
                  // $field_html .= '&nbsp;<span class="required">*</span>';
                }
              $field_html .= '</label>';
              $field_html .= '<input id="' . esc_attr( $key ) . '" name="' . esc_attr( $key ) . '" class="form-control" type="' . esc_attr( $field['type'] ) . '" value="' . esc_attr( $field['value'] ) . '" size="30" ' . ( $field['required'] ? 'required' : '' ) . ' />';
            $field_html .= '</div>';
          ...
          if ( wc_review_ratings_enabled() ) {
            $comment_form['comment_field'] = '<div class="comment-form-rating mb-3"><label for="rating">' . esc_html__( 'Rate This Product', 'woocommerce' ) . '</label><select name="rating" id="rating" required>
