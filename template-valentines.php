    ...
    <?php else : ?>
      ...
      <div id="holiday-gift-guide-banner" class="row text-center py-10 py-lg-5 mb-5" <?php if ($feat_img_url) { ?>style="background-image: url('<?php echo esc_url($feat_img_url); ?>');"<?php } ?>>
        <div class="col-md-6 offset-md-3 col-lg-4 offset-lg-4">
          <h1 class="text-center text-white mb-4"><?php echo get_the_title(); ?></h1>
          <form id="show-products-under-dollar-amount">
            <div class="form-group text-center text-white">
              <label for="rangeInput">Show Products Under Dollar Amount</label>
              <input type="range" class="form-control-range" id="rangeInput" min="25" max="<?php echo esc_attr(get_field('range_output')); ?>" step="10" value="<?php echo esc_attr(get_field('range_output')); ?>"/>
              <div>
                <button id="btn-range-minus" type="button" class="btn btn-range-minus text-white">
                  <span class="sr-only">Minus</span>
                  <i class="fa fa-minus" aria-hidden="true"></i>
                </button>
                <output class="mt-2" id="rangeOutput">$<?php echo esc_html(get_field('range_output')); ?></output>
                <button id="btn-range-plus" type="button" class="btn btn-range-plus text-white" disabled>
      ...
      <?php if ($query->have_posts()) : ?>
        ...
        <div class="row">
          <?php while ($query->have_posts()) : $query->the_post(); ?>
            ...
            $product = get_post();
            if (!$checked_access) {
              $can_access_vip_product = radical_can_access_vip_product_product_cat(get_the_ID());
              $checked_access = true;
            }
            ?>
